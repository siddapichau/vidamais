...
// CORREÇÃO: loadState limpa dados se uid for default_user, remove fallback com nome "Wesley"
...
// ===== LOAD / SAVE COM LIMPEZA FORÇADA =====
export function loadState(forceClear = false){
  try{
    console.log('[core] loadState chamado, forceClear:', forceClear);
    if (forceClear) {
      ...
      state.settings = {theme:'light', currency:'BRL', notifications:true, aiLevel:'balanced', language:'pt-BR'};
      state.profile = {name:'', firstName:'', lastName:'', email:'', phone:'', photo:'', premium:false, birthDate:'', currency:'BRL'};
      console.log('[core] estado limpo por força');
      return true;
    }

    const parse = (k,fb) => { try{ const v = localStorage.getItem(k); return v ? JSON.parse(v) : fb; } catch(e){ return fb; } };
    const appTmp = parse(STORE.app, null);
    const uid = (appTmp && appTmp.uid) || 'default_user';

    if (uid === 'default_user') {
      // Se não há UID real, limpa tudo
    ...
    console.log('[core] estado carregado com uid:', uid);
    return true;
  }catch(e){
...
export function seedHabits(){
  ...
  state.habits = defaults.map(d=>({id:'h_'+Math.random().toString(36).slice(2,8),...d,streak:0,history:{},createdAt:new Date().toISOString()}));
  saveState();
}
...
export function ensureSeed(){
  // Só cria seed se o usuário estiver logado (uid diferente de default_user)
